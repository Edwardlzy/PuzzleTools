...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Return True iff sudoku can never be solved.
        ...
        if "*" not in symbols:
            ...
            for i in range(len(symbols)):
                if symbols[i] == '*':
                    unknown.append(i)
            for elem in unknown:
                available = (self._symbol_set - (self._row_set(elem) | self._column_set(elem) |
                                                 self._subsquare_set(elem)))
                if not available:
                    return True
        return False
...
if __name__ == "__main__":
    ...
    print("time to solve 9x9 using depth_first: {}s\n".format(
        end - start))
    print(sol)
